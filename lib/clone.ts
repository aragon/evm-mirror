...
import { ChainInfo, CompilerMeta, ContractSourcesWithMeta } from "./types.ts";
...
export async function cloneContract(
  ...
  console.log(
    gray(
      `\nGenerated ${bold("foundry.toml")} (solc ${
        parseCompilerVersion(meta.compilerVersion)
      }, optimizer ${meta.runs} runs)`,
    ),
  );

  // Generate remappings.txt - use metadata if available, otherwise detect from paths
  const remappings = meta.remappings.length > 0
    ? meta.remappings
    : detectRemappingsFromPaths(paths);
  ...
  if (remappings.length > 0) {
    ...
    console.log(
      gray(
        `Generated ${bold("remappings.txt")} (${remappings.length} ${
          remappings.length === 1 ? "entry" : "entries"
        })`,
      ),
    );
  }
